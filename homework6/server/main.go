...
import (
	...
	"gorm.io/gorm"
	"homework/homework6/api"
	"homework/homework6/database"
	showInterface "homework/homework6/interface"
	"log"
	"net/http"
	"strconv"
)
...
func connectDb() *gorm.DB { //
	conn, err := gorm.Open(mysql.Open("root:fly990712@tcp(127.0.0.1:3306)/testdb")) //与数据库建立连接
...
func main() {
	...
	var ShowServer showInterface.ServeInterface = database.NewDbShow(conn)
	// 和数据库对接 完成
	//创建20个数据
	for i := 0; i < 20; i++ {
